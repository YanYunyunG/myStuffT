...
public class StrategyRunner implements Runnable, ErrorListener {
	...
	public void error(int errorCode, String errorMsg) {
		...
		boolean isRestoredWithDataLost = (errorCode == 1101);// "Connectivity
																// between IB
																// and TWS has
																// been
																// restored-
	...
	public StrategyRunner(Strategy strategy) throws IOException,
	...
	private void getHistoricalData() throws InterruptedException {
		...
		// Second request for the history data with different bar size.
	/*	if ( strategy.getSecondBarSize() != null )
		{
			...
			barSize = strategy.getSecondBarSize().toIBText();
			if (strategy.getSecondBarSize() != null) {
				String secondHistRequestDuration = strategy.getSecondBarSize()
				.getHistRequestDuration();
				trader.getAssistant().getHistoricalData(strategy.getId(), contract,
						timeNow, secondHistRequestDuration, barSize, priceType,
						onlyRTHPriceBars, 2);

				synchronized (trader) {
					while (!secondQH.getIsHistRequestCompleted()) {
						trader.wait();
					}
				}
			}
...
*/
	...
	public void run() {
		try {
			...
			new BarFactory(strategy.getName(), strategy.getQuoteHistory(),
					strategy.getBarSize());

	//		if (strategy.getSecondBarSize() != null) {
		//		new BarFactory(strategy.getName() + "_secondBar", strategy
			//			.getSecondHQ(), strategy.getSecondBarSize());
		//	}
