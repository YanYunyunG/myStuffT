...
public class Trader extends IBWrapperAdapter {
    ...
    public void historicalData(int reqId, String date, double open, double high, double low, double close, int volume, int count, double WAP, boolean hasGaps) {
        try {
            ...
            boolean addForSecondQH = false;
            
            if ( qh.getIsHistRequestCompleted() ) 
            	//all the data has been added into the first qh, now for the second one
            {
            	qh = traderAssistant.getStrategy(reqId).getSecondHQ();
            	addForSecondQH = true;
            }
            if (date.startsWith("finished")) {
                ...
                if ( addForSecondQH )
                {
                	priceBarDate = (Long.parseLong(date) + strategy.getSecondBarSize().toSeconds()) * 1000;
                }
                ...
                if (priceBarDate <= System.currentTimeMillis()) { //is the bar completed?
                	strategy.validateIndicators(addForSecondQH);
                }
            }
        } catch (Throwable t) {
            // Do not allow exceptions come back to the socket -- it will cause disconnects
    ...
    public void realtimeBar(int strategyId, long time, double open, double high, double low, double close, long volume, double wap, int count) {
        try {
            ...
            if ( traderAssistant.getStrategy(strategyId).getSecondBarSize() != null )
            {
            	 QuoteHistory secondQHUpdate = traderAssistant.getStrategy(strategyId).getSecondHQ();
            	 secondQHUpdate.update(open, high, low, close, volume);
            }
